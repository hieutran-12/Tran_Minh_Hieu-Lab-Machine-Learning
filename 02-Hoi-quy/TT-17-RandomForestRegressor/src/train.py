...
Pipeline ĐẦY ĐỦ, END-TO-END theo checklist (bản v2 — đã tối ưu):

  1.  Nạp dữ liệu + KIỂM TRA CHẤT LƯỢNG DỮ LIỆU (null, trùng lặp, giá trị âm/bất
      thường) trước khi train — không "tin mù" vào file CSV đầu vào.
  2.  EDA: giá theo days_left, boxplot theo class/airline.
  3.  Pipeline tiền xử lý (OneHotEncoder cho biến phân loại, không scale numeric).
  4.  Chia train/test CÓ PHÂN TẦNG (stratify) theo `class` — biến chi phối mạnh
      nhất — để tỉ lệ Economy/Business giống nhau ở cả 2 tập.
  5.  Baseline: Dummy + Linear + 1 Decision Tree (TT-16).
  6.  Random Forest mặc định + oob_score_.
  7.  TINH CHỈNH SIÊU THAM SỐ (RandomizedSearchCV, k-fold CV) — không dùng
      tham số mặc định "đoán bừa" nữa. Tìm trên một tập con để tiết kiệm thời
      gian, sau đó refit trên toàn bộ train với cấu hình tốt nhất.
  8.  Cross-validation 5-fold cho model cuối (không chỉ 1 lần train/test split
      — tránh kết luận may rủi do một cách chia dữ liệu).
  9.  Đường cong RMSE theo n_estimators = 10..500 → tìm điểm bão hoà.
  10. Permutation importance (KHÔNG dùng feature_importances_ mặc định) — VÀ vẽ
      SO SÁNH trực tiếp với feature_importances_ (Gini) để CHỨNG MINH bằng số
      liệu thực tế claim "Gini importance thiên vị biến nhiều mức".
  11. Partial Dependence Plot cho days_left → định lượng tiết kiệm khi mua sớm.
  12. Khoảng dự báo 10–90% từ các cây + đo % ca giá thật rơi trong khoảng.
  13. THÍ NGHIỆM NGOẠI SUY days_left=100 — chạy trên NHIỀU hồ sơ khách đại
      diện (không phải 1 dòng ngẫu nhiên) để kết luận đáng tin cậy hơn.
  14. KIỂM TRA RÒ RỈ DỮ LIỆU bằng shuffled-target sanity check: train lại trên
      target đã xáo trộn ngẫu nhiên → R² phải rơi về gần 0. Nếu không, có rò rỉ.
  15. So sánh với XGBoost Regressor (TT-19), có early stopping.
  16. Lưu model + toàn bộ metadata (feature list, phiên bản thư viện, siêu tham
      số tốt nhất, thời gian chạy) để tái lập kết quả.

Cách chạy:
    python src/train.py --data data/Clean_Dataset.csv
    python src/train.py --data data/Clean_Dataset.csv --fast      # chạy nhanh để debug
    python src/train.py --data data/Clean_Dataset.csv --no-tune   # bỏ qua bước tinh chỉnh

Kiểm thử (không cần dữ liệu thật, dùng dữ liệu giả lập cùng schema):
    pytest tests/ -v

Yêu cầu: đặt file dữ liệu gốc Clean_Dataset.csv vào thư mục data/
(cột dữ liệu: Unnamed: 0, airline, flight, source_city, departure_time,
